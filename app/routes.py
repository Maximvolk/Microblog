...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(_("Congratulations, henceforth you are a registered user!"))
        return redirect(url_for('login'))
...
def user(username):
    ...
    return render_template('user.html', title=username.capitalize(), user=user,
                           posts=posts.items, next_url=next_url, prev_url=prev_url)
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(_('Changes have been saved successfully.'))
